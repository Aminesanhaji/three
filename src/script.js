...
const posAray = new Float32Array(particlesCnt * 3);
// xyz, xyz, xyz ,xyz 

for(let i = 0; i < particlesCnt * 3; i++) {
    // posAray[i] = Math.random() - 0.5
    posAray[i] = (Math.random() - 0.5) * 5
}
...
renderer.setSize(sizes.width, sizes.height)
renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
renderer.setClearColor(new THREE.Color('#0F193B'), 0.5)
...
const tick = () =>
{
    ...
    if (mouseX > 0) {
        ...
        particlesMesh.rotation.y = -mouseX * (elapsedTime * 0.000009)
    }
